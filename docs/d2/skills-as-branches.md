...
Halo skills are distributed as git branches on the upstream repository. Applying a skill is a `git merge`. Updating core is a `git merge`. Everything is standard git.

This replaces the previous `skills-engine/` system (three-way file merging, `.halo/` state, manifest files, replay, backup/restore) with plain git operations and Claude for conflict resolution.

## How It Works

### Repository structure

The upstream repo (`qwibitai/halo`) maintains:

- `main` — core Halo (no skill code)
...
- `/setup`, `/debug`, `/update-halo`, `/customize`, `/update-skills`
...
- Live in the marketplace repo (`qwibitai/halo-skills`)
...
claude plugin install halo-skills@halo-skills --scope project
```
...
Halo's `.claude/settings.json` registers the official marketplace:
...
```
qwibitai/halo-skills/
  .claude-plugin/
    marketplace.json              # Plugin catalog
  plugins/
    halo-skills/              # Single plugin bundling all official skills
      .claude-plugin/
...
Multiple skills are bundled in one plugin — installing `halo-skills` makes all feature skills available at once. Individual skills don't need separate installation.
...
- Built into `/update-halo` — after merging main, optionally check for skill updates
...
1. Fork `qwibitai/halo` on GitHub (click the Fork button)
2. Clone your fork:
   ```bash
...
Users who previously ran `git clone https://github.com/qwibitai/halo.git` and have local customizations:

1. Fork `qwibitai/halo` on GitHub
2. Reroute remotes:
   ```bash
...
3. From this point, `origin` = their fork, `upstream` = qwibitai/halo
...
In both cases:
- Delete the `.halo/` directory (no longer needed)
...
This is the same as the existing `/update-halo` skill's merge path.

### Updating skills

Run `/update-skills` or let `/update-halo` check after a core update. For each previously-merged skill branch that has new commits, Claude offers to merge the updates.
...
# Create PR from my-fix to qwibitai/halo:main
```
...
4. Add the skill's SKILL.md to the marketplace repo (`qwibitai/halo-skills`)
...
1. Maintains a fork of Halo (e.g., `alice/halo`)
2. Creates `skill/*` branches on their fork with their custom skills
3. Creates a marketplace repo (e.g., `alice/halo-skills`) with a `.claude-plugin/marketplace.json` and plugin structure

### Adding a community marketplace

If the community contributor is trusted, they can open a PR to add their marketplace to Halo's `.claude/settings.json`:
...
Once merged, all Halo users automatically discover the community marketplace alongside the official one.
...
```bash
claude plugin install alice-skills@alice-halo-skills --scope project
```
...
- **Community skills use the same merge pattern.** The SKILL.md just points to a different remote:
  ```bash
  git remote add alice https://github.com/alice/halo.git
  git fetch alice skill/my-cool-feature
  git merge alice/skill/my-cool-feature
  ```
- **Users can also add marketplaces manually.** Even without being listed in settings.json, users can run `/plugin marketplace add alice/halo-skills` to discover skills from any source.
...
A flavor is a curated fork of Halo — a combination of skills, custom changes, and configuration tailored for a specific use case (e.g., "Halo for Sales," "Halo Minimal," "Halo for Developers").
...
AskUserQuestion: "Start with a flavor or default Halo?"
- Default Halo
- Halo for Sales — Gmail + Slack + CRM (maintained by alice)
- Halo Minimal — Telegram-only, lightweight (maintained by bob)

If a flavor is chosen:

```bash
git remote add <flavor-name> https://github.com/alice/halo.git
...
- `upstream` — `qwibitai/halo` (core updates)
...
- **GitHub topics** — flavor forks can tag themselves with `halo-flavor` for searchability
...
- `.halo/` state directory

Operational skills (`setup`, `debug`, `update-halo`, `customize`, `update-skills`) remain on main in `.claude/skills/`.
...
```
1. Fork qwibitai/halo on GitHub
2. git clone https://github.com/<you>/halo.git
3. cd halo
4. claude
5. /setup
```
...
- Check if `upstream` remote exists; if not, add it: `git remote add upstream https://github.com/qwibitai/halo.git`
- Check if `origin` points to the user's fork (not qwibitai). If it points to qwibitai, guide them through the fork migration.
- **Install marketplace plugin:** `claude plugin install halo-skills@halo-skills --scope project` — makes all feature skills available (hot-loaded, no restart)
...
The `.claude/skills/` directory on `main` retains only operational skills (setup, debug, update-halo, customize, update-skills). Feature skills (add-discord, add-telegram, etc.) live in the marketplace repo, installed via `claude plugin install` during `/setup` or `/customize`.
...
- **Marketplace repo** (`qwibitai/halo-skills`) — single Claude Code plugin bundling SKILL.md files for all feature skills
...
>
> We've simplified how skills work in Halo. Instead of a custom skills engine, skills are now git branches that you merge in.
>
...
> - No more `.halo/` state directory or skills engine
>
...
>
> **If you previously applied skills via the old system**, your code changes are already in your working tree — nothing to redo. You can delete the `.halo/` directory. Future skills and updates use the branch-based approach.
>
...
>
> **Want to run your own skill marketplace?** Maintain skill branches on your fork and create a marketplace repo. Open a PR to add it to Halo's auto-discovered marketplaces — or users can add it manually via `/plugin marketplace add`.
