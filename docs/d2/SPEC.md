...
│  │    • mcp__halo__* (scheduler tools via IPC)                │    │
│  │                                                                │    │
│  └──────────────────────────────────────────────────────────────┘    │
│                                                                       │
└───────────────────────────────────────────────────────────────────────┘
...
│   ├── halo.log               # Host stdout
│   └── halo.error.log         # Host stderr
│   # Note: Per-container logs are in groups/{folder}/logs/container-*.log
│
└── launchd/
    └── com.halo.plist         # macOS service configuration
```
...
export const CONTAINER_IMAGE = process.env.CONTAINER_IMAGE || 'halo-agent:latest';
...
- `{{PROJECT_ROOT}}` - Absolute path to your halo installation
...
Halo uses a hierarchical memory system based on CLAUDE.md files.
...
Halo has a built-in scheduler that runs tasks as full agents in their group's context.
...
Claude: [calls mcp__halo__schedule_task]
        {
...
The `halo` MCP server is created dynamically per agent call with the current group's context.
...
Halo runs as a single macOS launchd service.

### Startup Sequence

When Halo starts, it:
1. **Ensures container runtime is running** - Automatically starts it if needed; kills orphaned Halo containers from previous runs
...
<dict>
    ...
    <string>{{PROJECT_ROOT}}/logs/halo.log</string>
    <key>StandardErrorPath</key>
    <string>{{PROJECT_ROOT}}/logs/halo.error.log</string>
</dict>
</plist>
```
...
```bash
# Install service
cp launchd/com.halo.plist ~/Library/LaunchAgents/
...
|-------|-------|----------|
| No response to messages | Service not running | Check `launchctl list | grep halo` |
| "Claude Code process exited with code 1" | Container runtime failed to start | Check logs; Halo auto-starts container runtime but may fail |
...
- `logs/halo.log` - stdout
- `logs/halo.error.log` - stderr
