...
Adds Parallel AI MCP integration to Halo for advanced web research capabilities.
...
- **Non-blocking Design** - Uses Halo scheduler for result polling (no container blocking)

## Prerequisites

User must have:
1. Parallel AI API key from https://platform.parallel.ai
2. Halo already set up and running
...
Add Parallel AI MCP servers after the halo server:
```typescript
const mcpServers: Record<string, any> = {
  halo: ipcMcp
};
...
3. Create a polling scheduled task using `mcp__halo__schedule_task`:
   ```
   ...
   3. Send results to user with mcp__halo__send_message
   4. Use mcp__halo__complete_scheduled_task to mark this task as done
...
Verify the build:
```bash
echo '{}' | docker run -i --entrypoint /bin/echo halo-agent:latest "Container OK"
```
...
```bash
sleep 3
launchctl list | grep halo  # macOS
...
- Check task runs: `tail -f logs/halo.log | grep "scheduled task"`
...
5. Restart: `launchctl kickstart -k gui/$(id -u)/com.halo` (macOS) or `systemctl --user restart halo` (Linux)
