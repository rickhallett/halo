...
This skill adds Telegram support to Halo, then walks through interactive setup.
...
- Check bot is started: `tail -f logs/halo.log`
...
6. Rebuild: `npm run build && launchctl kickstart -k gui/$(id -u)/com.halo` (macOS) or `npm run build && systemctl --user restart halo` (Linux)
