...
<dict>
    ...
    <string>${projectRoot}/logs/halo.log</string>
    <key>StandardErrorPath</key>
    <string>${projectRoot}/logs/halo.error.log</string>
</dict>
</plist>`;
  ...
  try {
    ...
    serviceLoaded = output.includes('com.halo');
  } catch {
...
/**
 * Kill any orphaned halo node processes left from previous runs or debugging.
...
function killOrphanedProcesses(projectRoot: string): void {
  try {
    ...
    logger.info('Stopped any orphaned halo processes');
  } catch {
...
function setupSystemd(
  ...
  if (runningAsRoot) {
    unitPath = '/etc/systemd/system/halo.service';
    ...
    unitPath = path.join(unitDir, 'halo.service');
...
StandardOutput=append:${projectRoot}/logs/halo.log
StandardError=append:${projectRoot}/logs/halo.error.log
...
WantedBy=${runningAsRoot ? 'multi-user.target' : 'default.target'}`;
  ...
  // Kill orphaned halo processes to avoid channel connection conflicts
  ...
  try {
    execSync(`${systemctlPrefix} enable halo`, { stdio: 'ignore' });
  } catch (err) {
    logger.error({ err }, 'systemctl enable failed');
  }

  try {
    execSync(`${systemctlPrefix} start halo`, { stdio: 'ignore' });
  } catch (err) {
  ...
  let serviceLoaded = false;
  try {
    execSync(`${systemctlPrefix} is-active halo`, { stdio: 'ignore' });
    serviceLoaded = true;
  } catch {
    // Not active
  }
...
function setupNohupFallback(
  ...
  const wrapperPath = path.join(projectRoot, 'start-halo.sh');
  const pidFile = path.join(projectRoot, 'halo.pid');

  const lines = [
    '#!/bin/bash',
    '# start-halo.sh — Start NanoClaw without systemd',
    ...
    `  >> ${JSON.stringify(projectRoot + '/logs/halo.log')} \\`,
    `  2>> ${JSON.stringify(projectRoot + '/logs/halo.error.log')} &`,
    '',
    `echo $! > ${JSON.stringify(pidFile)}`,
    'echo "NanoClaw started (PID $!)"',
    `echo "Logs: tail -f ${projectRoot}/logs/halo.log"`,
  ];
